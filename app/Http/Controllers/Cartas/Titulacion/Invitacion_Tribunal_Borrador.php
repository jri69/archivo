...
class Invitacion_Tribunal_Borrador extends Fpdf

{
    ...
    public function informe($data)
    {
        ...
        foreach ($cartas as $key => $carta) {
            ...
            $this->fpdf->SetFont('Arial', 'B', 9);
            $this->fpdf->MultiCell($this->width, $this->space, utf8_decode("Ref.:  Borrador de tesis de maestría"), 0, 'C', 0);
            $this->fpdf->Ln(2);

            // CONTENIDO
            $contenido = [
                'first' => "Por sus conocimientos y antecedentes profesionales, nos es grato designarlo miembro del tribunal del trabajo final de grado titulado <" . $titulacion->tesis . "> presentado por " . $nombre_estudiante . ", " . $nombre_programa . ".",
                'second' => "Seguro de su aceptación, adjunto a este documento una copia del borrador elaborado por el mencionado maestrante.  El plazo máximo estipulado por el Reglamento General de Postgrado para la revisión del trabajo final de grado es de 30 días a partir de la fecha.",
                'third' => "El informe de evaluación del trabajo final de grado debe contener un análisis de los siguientes aspectos:",
            ];
            $this->fpdf->SetFont('Arial', '', 10);
            $this->fpdf->MultiCell($this->width, $this->space, utf8_decode("De nuestra mayor consideración:"), 0, 'L', 0);
            $this->fpdf->Ln(5);
            $this->fpdf->SetFont('Arial', '', 9);
            ...
            $this->fpdf->Ln(8);
            $this->WriteText("Agradeciendo de antemano su gentil colaboración, saludo a Ud. con las consideraciones más distinguidas.");
            $this->fpdf->Ln(8);
            ...
            $this->fpdf->MultiCell($this->width, 4, utf8_decode("Adjunto: Borrador."), 0, 'L', 0);
            // FONT BOLD
        }
